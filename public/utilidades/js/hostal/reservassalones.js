...
$(document).ready(function() {
    TraerAmbienteSalon()
    FechaSelectReservaSalon()
});

document.getElementById('addreservassalone').addEventListener('click', function() {
    
    $('#form_tabs').empty();
        var addFormCardSalon = `
        <div class="card-header" style="width: 100%;">
            <h3 class="card-title">AGREGAR RESERVAR SALON</h3>
        </div>
        <div class="card-body">
            <div class="datagrid">
                <div class="row">
                    <div class="col-12 col-sm-6" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Seleccionar Salon</label>
                            <select class="form-control" id="SeleccionarSalonInput">
                    ...
                    <div class="col-12 col-sm-6" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Tarifa</label>
                            <select class="form-control" id="SeleccionarTarifaInput">
                    ...
                    <div class="col-12 col-sm-3" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Precio</label>
                            <input class="form-control" id="PrecioSalonInput">
                        </div>
                    </div>                              
                    <div class="col-12 col-sm-3" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Fecha</label>
                            <input type="date" class="form-control" id="FechaIngresoSalonInput">
                        </div>
                    </div>
                    <div class="col-12 col-sm-3" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Hora Inicio</label>
                            <input type="time" class="form-control" id="HoraInicioSalonInput">
                        </div>
                    </div>
                    <div class="col-12 col-sm-3" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Hora Finalización</label>
                            <input type="time" class="form-control" id="HoraFinSalonInput" readonly>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-6" style="padding: 10px">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control convertmayusculas" id="NombreClienteReservasSalonInput">
                            <ul class="list-group" id="cliente-suggestions" style="display: none"></ul>
                        </div>
                        <div class="mb-3" id="telefono-div" style="display: none">
                            <label class="form-label required">Celular</label>
                            <input type="text" class="form-control convertmayusculas" id="TelefonoClienteReservasSalonInput">
                        </div>
                        <div class="form-check" id="agregar-cliente-checkbox-div" style="display: none">
                            <input class="form-check-input" type="checkbox" id="agregar-cliente-checkbox">
                            <label class="form-check-label" for="agregar-cliente-checkbox">Agregar Cliente si no existe</label>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-6" style="padding: 10px">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control convertmayusculas" id="EmpresaReservasSalonInput">
                            <ul class="list-group" id="empresa-suggestions" style="display: none"></ul>
                            <br>
                            <div class="form-check" id="agregar-empresa-checkbox-div" style="display: none">
                                <input class="form-check-input" type="checkbox" id="agregar-empresa-checkbox">
                                <label class="form-check-label" for="agregar-empresa-checkbox">Agregar Empresa si no existe</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-12" style="padding: 10px">
                        <div class="mb-3">
                            <label class="form-label required">Comentario Reserva</label>
                            <textarea class="form-control convertmayusculas" rows="4" id="ComentarioSalonInput"></textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card-footer">
            ...
            <button type="button" class="btn btn-primary" id="btn-registrar-reserva-salon">RESERVAR</button>
            </div>
        </div>
    `;        
    ...
    $('#NombreClienteReservasSalonInput').on('input', function() {
        let query = $(this).val();
        if (query.length > 2) {
            $.ajax({
                url: '/apihostal/get-clientes-reservas',
                method: 'GET',
                data: { query: query },
                success: function(data) {
                    $('#cliente-suggestions').empty().show();
                    if (data.length > 0) {
                        data.forEach(cliente => {
                            $('#cliente-suggestions').append(`<li class="list-group-item cliente-item" data-id="${cliente.id}">${cliente.NombreCliente}</li>`);
                        });
                        $('#agregar-cliente-checkbox-div').hide();
                        $('#telefono-div').hide();
                    } else {
                        $('#cliente-suggestions').hide();
                        $('#agregar-cliente-checkbox-div').show();
                        $('#agregar-cliente-checkbox').prop('checked', true);
                        $('#agregar-cliente-checkbox').prop('disabled', true); 
                        $('#telefono-div').show(); 
                    }
                }
            });
    ...
    $(document).on('click', '.cliente-item', function() {
        let clienteNombre = $(this).text();
        let clienteId = $(this).data('id');
        $('#NombreClienteReservasSalonInput').val(clienteNombre);
        $('#cliente-suggestions').hide();
        $('#agregar-cliente-checkbox-div').hide();
        $('#telefono-div').hide(); 
        $('#agregar-cliente-checkbox').prop('checked', false); 
        $('#agregar-cliente-checkbox').prop('disabled', false);
    });

    // Función para buscar empresas
    $('#EmpresaReservasSalonInput').on('input', function() {
        let query = $(this).val();
        if (query.length > 2) {
            $.ajax({
                url: '/apihostal/get-empresas-reservas',
                method: 'GET',
                data: { query: query },
                success: function(data) {
                    $('#empresa-suggestions').empty().show();
                    if (data.length > 0) {
                        data.forEach(empresa => {
                            $('#empresa-suggestions').append(`<li class="list-group-item empresa-item" data-id="${empresa.id}">${empresa.NombreEmpresa}</li>`);
                        });
                        $('#agregar-empresa-checkbox-div').hide();
                    } else {
                        $('#empresa-suggestions').hide();
                        $('#agregar-empresa-checkbox-div').show();
                        $('#agregar-empresa-checkbox').prop('checked', true);
                        $('#agregar-empresa-checkbox').prop('disabled', true);
                    }
                }
            });
        } else {
    ...
    $(document).on('click', '.empresa-item', function() {
        let empresaNombre = $(this).text();
        let empresaId = $(this).data('id');
        $('#EmpresaReservasSalonInput').val(empresaNombre);
        $('#empresa-suggestions').hide();
        $('#agregar-empresa-checkbox-div').hide();
        $('#agregar-empresa-checkbox').prop('checked', false);
        $('#agregar-empresa-checkbox').prop('disabled', false);
    });


    TraerAmbienteSalon()

    soloHora('HoraInicioSalonInput');              

    var tarifaSelect = document.getElementById('SeleccionarTarifaInput');
    var horaInicioInput = document.getElementById('HoraInicioSalonInput');
    var horaFinInput = document.getElementById('HoraFinSalonInput');

    function calcularHoraFin() {
        var horaInicio = horaInicioInput.value;
        var tarifa = tarifaSelect.value;

        if (horaInicio && tarifa) {
            var horasAgregar = tarifa === 'COMPLETA' ? 8 : 4;
            var [hours, minutes] = horaInicio.split(':');
            var fechaInicio = new Date();
            fechaInicio.setHours(parseInt(hours), parseInt(minutes));
            fechaInicio.setHours(fechaInicio.getHours() + horasAgregar);
            var horaFinal = fechaInicio.toTimeString().slice(0, 5);
            horaFinInput.value = horaFinal;
        }
    }

    tarifaSelect.addEventListener('change', calcularHoraFin);
    horaInicioInput.addEventListener('input', calcularHoraFin);

    $('#btn-registrar-reserva-salon').off('click').on('click', function(event) {
        event.preventDefault();
        // Recuperar valores de salón, tarifa, etc.
        var SeleccionarSalonInput = $('#SeleccionarSalonInput').val();
        var SeleccionarTarifaInput = $('#SeleccionarTarifaInput').val();
        var PrecioSalonInput = $('#PrecioSalonInput').val();
        var HoraInicioSalonInput = $('#HoraInicioSalonInput').val();
        var HoraFinSalonInput = $('#HoraFinSalonInput').val();
        var ComentarioSalonInput = $('#ComentarioSalonInput').val();
        var FechaIngresoSalonInput = $('#FechaIngresoSalonInput').val();
    
        // Recuperar valores de cliente y empresa
        var NombreClienteReservasSalonInput = $('#NombreClienteReservasSalonInput').val();
        var TelefonoClienteReservasSalonInput = $('#TelefonoClienteReservasSalonInput').val();
        var AgregarClienteCheckbox = $('#agregar-cliente-checkbox').is(':checked');
        
        var EmpresaReservasSalonInput = $('#EmpresaReservasSalonInput').val();
        var AgregarEmpresaCheckbox = $('#agregar-empresa-checkbox').is(':checked');
    
        // Crear objeto con todos los datos
        const dataToSend = {
            SeleccionarSalonInput: SeleccionarSalonInput,
            SeleccionarTarifaInput: SeleccionarTarifaInput,
            PrecioSalonInput: PrecioSalonInput,
            HoraInicioSalonInput: HoraInicioSalonInput,
            HoraFinSalonInput: HoraFinSalonInput,
            ComentarioSalonInput: ComentarioSalonInput,
            FechaIngresoSalonInput: FechaIngresoSalonInput,
            
            // Datos de cliente
            NombreClienteReservasSalonInput: NombreClienteReservasSalonInput,
            TelefonoClienteReservasSalonInput: TelefonoClienteReservasSalonInput,
            AgregarClienteCheckbox: AgregarClienteCheckbox, // Indicar si se está agregando un nuevo cliente
            
            // Datos de empresa
            EmpresaReservasSalonInput: EmpresaReservasSalonInput,
            AgregarEmpresaCheckbox: AgregarEmpresaCheckbox  // Indicar si se está agregando una nueva empresa
        };
    
        // Enviar los datos por AJAX
        $.ajax({
            url: '/apihostal/registrar-reserva-salon-ambiente',
            method: 'POST',
            data: dataToSend,
            success: function(response) {
                MostrarAmbientes();
                filtrarDatosReservaSalon();
                MostrarMensaje("Reserva Creado Exitosamente", "success");
                CanvasTime();
            },
...
function FechaSelectReservaSalon() {
    var today = new Date();
    ...
    $('#MesReservaSalon').empty();
    $('#AnioReservaSalon').empty();

    for (var month = 0; month < 12; month++) {
        $('#MesReservaSalon').append('<option value="' + (month + 1) + '">' + monthsOfYear[month] + '</option>');
    }
    $('#MesReservaSalon').val(currentMonth + 1);

    var startYear = currentYear - 6;
    var endYear = currentYear;
    for (var year = startYear; year <= endYear; year++) {
        $('#AnioReservaSalon').append('<option value="' + year + '">' + year + '</option>');
    }
    $('#AnioReservaSalon').val(currentYear);

    function updateDaySelector() {
        var selectedMonth = parseInt($('#MesReservaSalon').val());
        var selectedYear = parseInt($('#AnioReservaSalon').val());
        var daysInMonth = new Date(selectedYear, selectedMonth, 0).getDate();
        $('#DiaReservaSalon').empty();
        for (var day = 1; day <= daysInMonth; day++) {
            $('#DiaReservaSalon').append('<option value="' + day + '">' + day + '</option>');
        }
        if (currentDay > daysInMonth) {
            $('#DiaReservaSalon').val(daysInMonth);
        } else {
            $('#DiaReservaSalon').val(currentDay);
        }
    }

    updateDaySelector();

    $('#DateReservaSalon').on('change', function() {
        var selectedValue = $(this).val();
        switch (selectedValue) {
            case 'DiarioReservaSalon':
                $('#DiaReservaSalonContainer').show();
                $('#MesReservaSalonContainer').show();
                $('#AnioReservaSalonContainer').show();
                $('#FechaInicioContainerReservaSalon').hide();
                $('#FechaFinContainerReservaSalon').hide();
                break;
            case 'MensualReservaSalon':
                $('#DiaReservaSalonContainer').hide();
                $('#MesReservaSalonContainer').show();
                $('#AnioReservaSalonContainer').show();
                $('#FechaInicioContainerReservaSalon').hide();
                $('#FechaFinContainerReservaSalon').hide();
            ...
            case 'RangoReservaSalon':
                $('#DiaReservaSalonContainer').hide();
                $('#MesReservaSalonContainer').hide();
                $('#AnioReservaSalonContainer').hide();
                $('#FechaInicioContainerReservaSalon').show();
                $('#FechaFinContainerReservaSalon').show();
                break;
            default:
                $('#DiaReservaSalonContainer').show();
                $('#MesReservaSalonContainer').show();
    ...
    $('#MesReservaSalon, #AnioReservaSalon').on('change', function() {
        updateDaySelector();
        filtrarDatosReservaSalon();
    });

    $('#DiaReservaSalon').on('change', function() {
        filtrarDatosReservaSalon();
    });

    $('#FechaInicioContainerReservaSalon').on('change', function() {
        filtrarDatosReservaSalon();
    });

    $('#FechaFinContainerReservaSalon').on('change', function() {
        filtrarDatosReservaSalon();
    });

    $('#TipoReservaSalon').on('change', function() {
        filtrarDatosReservaSalon();
    });

    $('#Salones').on('change', function() {
        filtrarDatosReservaSalon();
    });

    $('#DateReservaSalon').trigger('change');

}

function filtrarDatosReservaSalon() {
    var tipoFiltro = $('#DateReservaSalon').val();
    var dia = $('#DiaReservaSalon').val();
    var mes = $('#MesReservaSalon').val();
    var anio = $('#AnioReservaSalon').val();
    var fechaInicio = $('#fechaInicioReservaSalon').val();
    var fechaFin = $('#fechaFinReservaSalon').val();
    var TipoReservaSalon = $('#TipoReservaSalon').val();
    var Salones = $('#Salones').val();

    $.ajax({
        url: '/apihostal/filtrar-datos-reservas-salon',
        method: 'GET',
        data: {
...
function mostrarResultadosReservasSalon(filteredData) {
    $('#tabla-Reserva-salones tbody').empty();
    $.each(filteredData.reservas, function(index, reserva) {
        var row = '<tr>' +
            '<td hidden>' + reserva.id + '</td>' +
            '<td style="font-weight: bold;">' + reserva.Codigosalon + '</td>' +
            '<td style="font-weight: bold;">' + reserva.salon.Nombre_salon + '</td>' +
            '<td style="font-weight: bold;">' + reserva.hora_ingreso_salon + '</td>' +
            '<td style="font-weight: bold;">' + reserva.hora_ingreso + '</td>' +
            '<td style="font-weight: bold;">' + reserva.hora_salida + '</td>' +
            '<td style="font-weight: bold;">' + reserva.Estado + '</td>' +
        '</tr>';

        $('#tabla-Reserva-salones tbody').append(row);
    });

    $('#tabla-Reserva-salones tbody').off('click').on('click', 'tr', function() {
        $('#tabla-Reserva-salones tbody tr').removeClass('selected-row');
        $(this).addClass('selected-row');
        var id = $(this).find('td:first').text();
        $.ajax({
            url: '/apihostal/get-reserva-salon-seleccionado/' + id,
            type: 'GET',
            dataType: 'json',
            success: function(data) {
...
function InformacionReservaSalon(data){
    $.ajax({
        ...
        success: function(data) {
                        ...
                        <tr>
                            <td style="width: 10%">
                                ${allconsumo.ocupado === 'false' 
                                    ...
                                    : `<span class="badge badge-outline text-red" id="btn-pagar-consumo-reserva" data-bs-toggle="modal" data-bs-target="#ModalCerrarConsumoReservaSalon"  data-id="${allconsumo.id}" style="cursor: pointer;">NO</span>`}
                            </td>
            ...
            TotalProduct.innerHTML = `
                                ...
                                <div class="card-header">
                                    <ul class="nav nav-tabs card-header-tabs nav-fill" data-bs-toggle="tabs" role="tablist">
                                        ...
                                        ? '<li class="nav-item dropdown">'+
                                                ...
                                                '<a class="dropdown-item" href="#tabs-servicio-data-salon" data-bs-toggle="tab" aria-selected="false" role="tab" tabindex="-1">Data Display</a>'+
                                                '</div>'+
                                            '</li>'+
                                            '<li class="nav-item" role="presentation">'+
                                                                ...
                                                                <div class="col-12 col-md-12">
                                                                    <div class="mb-12 row">                                                                        
                                                                        <label class="col-12 col-form-label" style="font-weight: bold">
                                                                            ${data[0].Estado != 'COMPLETO' 
                                                                                ? `<a class="form-controll" id="btn-registrar-adelanto-salon" data-bs-toggle="modal" data-bs-target="#modal-adelanto-reserva-salon">Registrar Adelanto</a>` 
                                                                                : ''}
                                                                        </label>
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <div class="card-footer">
                                                                <div class="d-flex justify-content-end">
                                                                    ${data[0].Estado === 'PENDIENTE' 
                                                                        ? `<button type="button" class="btn btn-primary" id="btn-concluir-reserva-salon" data-id="${data[0].id}">CONCLUIR SALON</button>` 
                                                                        : ''}
                                                                </div>
                                                            </div>
                                                        </div>
                ...
                $('#table-consumos-reserva').off('click').on('click', '#btn-pagar-consumo-reserva', function() {
                    const consumoID = $(this).data('id'); 
                    $('#btn-confirmar-modal-consumo-reserva-salon').data('id', consumoID);
                ...
                $('#btn-concluir-reserva-salon').off('click').on('click', function(event) {
                    event.preventDefault();
                    const id = data[0].id;
                    $.ajax({
                        url: '/apihostal/concluir-reserva-salon/'+id,
                        method: 'GET',
                        success: function(data) {
                            MostrarAmbientes();
                            CanvasTime();
                            MostrarMensaje("Reserva Mandado Ambiente Exitosamente", "success");
                        },
                        error: function(jqXHR, textStatus, errorThrown) {                        
                ...
                $(document).on('click', '#btn-registrar-adelanto-salon', function(event) {
                    event.preventDefault();
                    $('#modal-adelanto-reserva-salon').modal('show');    
                            
                    $('#btn-registrar-modal-adelanto-reserva-salon').off('click').on('click', function(event) {
                        event.preventDefault();
                        var dataToSend = {
                            idreservaSalon: data[0].id,
                            TipoAdelanto: $('#TipoAdelanto-salon').val(),
                            MontoAdelanto: $('#MontoAdelanto-salon').val(),
                        };
                                        ...
                                        success: function (consumo) {
                                            ...
                                            DivBotones.innerHTML = `
                                                <div class="col-md-6 col-lg-12 d-flex justify-content-between">
                                                    ...
                                                    <button type="button" data-bs-toggle="modal" data-bs-target="#ModalCerrarConsumoReservaSalon" class="btn btn-danger" data-id="${IdConsumo}" id="btnCerrarConsumo">Cerrar Consumo</button>
                                                </div>
                                            `;
...
function agregarEventosReservaTabla() {
    $('#tabla-Reserva-salones tbody tr').hover(function() {
        $(this).addClass('hovered');
    }, function() {
        $(this).removeClass('hovered');
    });
    $('#tabla-Reserva-salones tbody tr').click(function() {
        $('#tabla-Reserva-salones tbody tr').removeClass('tableproducseleccionado');
        $(this).addClass('tableproducseleccionado').siblings().removeClass('tableproducseleccionado');
    });
}

function MostrarDivTotal(total){
    $('#TotalReserva').text('Bs. '+total);
}
...
function TraerAmbienteSalon() {
    $.ajax({
        url: '/apihostal/get-ambiente-salon',
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            var selectSalon = $('#SeleccionarSalonInput');
            selectSalon.empty();
            selectSalon.append('<option value="">Seleccione un salón</option>');
            data.forEach(function(salon) {
                selectSalon.append('<option value="' + salon.id + '">' + salon.Nombre_salon + '</option>');
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('Error al cargar los salones:', textStatus);
        }
    });
}

function soloHora(inputId) {
    const inputElement = document.getElementById(inputId);
    inputElement.addEventListener('input', function (e) {
        let value = e.target.value;
        let updatedValue = value.split(':')[0] + ":00";
